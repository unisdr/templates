class Tooltip {
  ...
  checkHorizontalBounding(bounds) {
    ...
    if (bounds.right > windowWidth && bounds.left < 0) {
      this.tooltip.style.width = `calc(${windowWidth}px * 0.8)`;
  ...
  moveTooltipLeft(bounds, windowWidth) {
      ...
      console.log("window/dialog Width" + windowWidth);
      console.log("tooltip bounds.left" + bounds.left);
      console.log("tooltip bounds.right" + bounds.right);
      console.log("tooltip bounds.width" + bounds.width);
      ...
      if (dialog) {
        console.log("dialog");
        let dialogBounds = dialog.getBoundingClientRect();
        console.log("dialog bounds.left" + dialogBounds.left);
        translateAmount = (Math.round(bounds.left) - Math.round(dialogBounds.left));
        this.tooltip.style.transform = `translateX(calc(-${translateAmount}px))`;
      } else {
        console.log("page");
